...
site under `docs/` must be able to do — 86 capabilities benchmarked against
...
`agents/skills/` holds the seven operating skills (`author-question`,
`author-plan`, `author-playbook`, `author-blog`, `fix-site-health`,
`fix-functional`, `build-capability`). `AgentManifest.load` resolves
`skills: [name]` against `agents/skills/<name>/SKILL.md` first, so genome and
skills travel together; `skills_dir: skills` in the organisation file points
chartered offices at the same folder. `harness.agent_manifest` fails if a skill
file exists that the CTO genome never lists, so a new operating skill is added
to `frontend-harness.yaml`'s `skills:` in the same commit.

The **`blog` capability group** is the marketing/SEO surface (`docs/blog/`), and
it is where the CMO earns its charter. `blog.count` is a growth target
(`required: false`, target 100) that reads honestly as N/100 until it fills; the
per-post checks (`blog.registry`, `blog.schema`, `blog.engagement`, `blog.depth`,
`blog.distinct`, `blog.index_filter`) hold every post that exists to the
GreatFrontend engagement bar — a byline and reading time, a table of contents, a
"keep reading" block, and a CTA into the practice bank, all filterable on the
index. The CMO office is chartered **explicitly** in `organisation.yaml` (a given
charter skips the evolver), with the `author-blog` skill, a `not-ai-slop` Jev
gate on every draft, and `verify:blog` — the site's own blog checker scoped to
the file the cell wrote — as its verifier. `docs/blog/drafts/` holds raw,
unpublished source notes and is excluded. `docs/_data/blog.yml` is generated by
`scripts/sync-blog-registry.py` — never hand-edit it.
