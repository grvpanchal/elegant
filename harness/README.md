...
| `bank.rendered` | 3 | yes | incremental | all | Every question row Jekyll writes must survive the browser's parse, and the page's own count must match. Two questions were in the HTML and gone from the DOM because a quote in a summary broke an attribute; the file count and the filter check both said fine. |
...
| `workspace.shortcuts` | 2 | yes | cumulative | all | Ctrl/Cmd+Enter must run the question's tests from the editor, and the page must say so. Every editor a candidate has used runs on that chord. |
...
| check | weight | required | scope | threshold | fails when |
|---|---|---|---|---|---|
| `blog.count` | 2 | no | cumulative | `100` | docs/blog/ is the growth target: >= `100` published posts. A marketing blog is a volume play. |
| `blog.registry` | 2 | yes | incremental | all | docs/_data/blog.yml must exist and match docs/blog/*.md one-for-one (generated by scripts/sync-blog-registry.py). |
| `blog.schema` | 2 | yes | incremental | all | Every post's front matter must satisfy blog_schema (missing keys, a bad category, a non-integer reading time are failures). |
