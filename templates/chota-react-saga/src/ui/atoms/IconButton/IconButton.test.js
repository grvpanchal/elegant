...
describe('<IconButton />', () => {
  ...
  it('Renders with variant class', () => {
    const { container } = render(
      <TestProvider>
        <IconButton iconName="edit" variant="clear" size="16" alt="edit" />
      </TestProvider>,
    );
  ...
  it('Renders with custom color', () => {
    render(
      <TestProvider preloadedState={{ config: { theme: 'light', name: 'Test', lang: 'en', themeMode: 'light' } }}>
  ...
  it('Uses theme color when no color is provided and theme is dark', () => {
    render(
      <TestProvider preloadedState={{ config: { theme: 'dark', name: 'Test', lang: 'en', themeMode: 'dark' } }}>
        <IconButton iconName="edit" alt="edit" size="16" />
      </TestProvider>,
    );
    const img = screen.getByAltText('edit');
  ...
  it('Renders with alt prop', () => {
    render(
      <TestProvider>
        <IconButton iconName="edit" alt="edit button" size="16" />
      </TestProvider>,
    );
    const img = screen.getByAltText('edit button');
